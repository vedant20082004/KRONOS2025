    ...
    <div className="min-h-screen bg-black text-slate-200 overflow-hidden">
      ...
      <header className="container mx-auto py-12 relative z-10">
        <motion.h1
          ...
          className="text-5xl md:text-7xl font-bold text-center tracking-wider font-['Orbitron'] text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-red-500"
        >
          EVENTS
        </motion.h1>
      ...
      <main className="container mx-auto px-4 pb-16 relative z-10">
        <AnimatePresence mode="wait">
          ...
          >
            <EventList events={eventsToShow} onEventClick={handleEventClick} activeSection={activeSection} />
          </motion.div>
        </AnimatePresence>

        <AnimatePresence>
          {selectedEvent && (
            <EventDetail event={selectedEvent} onClose={handleCloseDetail} activeSection={activeSection} />
          )}
        </AnimatePresence>
      </main>

      {/* HUD elements */}
      <div className="fixed bottom-4 left-4 text-xs text-blue-400 font-mono opacity-70">KRONOS_OS v3.4.2</div>
