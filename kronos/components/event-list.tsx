...
export default function EventList({ events, onEventClick, activeSection }: EventListProps) {
  const container = {
    hidden: { opacity: 0 },
    show: {
      opacity: 1,
      ...
      {events.map((event) => (
        <motion.div
          ...
          whileHover={{
            ...
            boxShadow: accentColor === "blue" ? "0 0 25px rgba(59, 130, 246, 0.3)" : "0 0 25px rgba(239, 68, 68, 0.3)",
          }}
        ...
        >
          <div className="relative h-48 overflow-hidden">
            ...
            <div className="absolute top-0 left-0 w-full p-3 flex justify-between items-start">
              <div className={`h-1 w-12 bg-${accentColor}-500/70 rounded-full`}></div>
              <div className={`h-1 w-6 bg-${accentColor}-500/70 rounded-full`}></div>
            </div>
          </div>
          ...
          <div className="p-4 relative">
            ...
            <div className={`text-xs text-${accentColor}-400 font-mono`}>{event.date}</div>
